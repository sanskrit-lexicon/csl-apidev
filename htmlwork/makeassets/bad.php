...
function html($key,$lnum,$data) {
 ...
 #return $table;
 # expect table has 4 lines, for YAT MD
 $tablines = explode("\n",$table); 
 if (count($tablines) != 4) { // YAT,MD
  echo "html ERROR 1: actual # lines in table = " . count($tablines) ."\n";
  exit(1);
 }
 $info = $tablines[1]; // YAT,MD
 $body = $tablines[2]; // YAT,MD
 # adjust body  YAT,MD 
 //$body = preg_replace('|<td.*?>|','',$body);
 //$body = preg_replace('|</td></tr>|','',$body);
 $body = preg_replace('|<br/> *$|','',$body); // YAT may have 2 br at end
 $body = preg_replace('|<br/> *$|','',$body); // YAT may have 2 br at end
