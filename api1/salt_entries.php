...
function saltEntriesCall() {
  ...
  if (isset($_GET['callback'])) {       // JSONP, like getsuggest.php
    // Shared whitelist+reply guard (H4212). (The whitelist is the real
    // control: a JSONP body is served as JavaScript, where htmlentities
    // alone would not neutralise an arbitrary callback. htmlentities()
    // remains inside the helper as defence-in-depth + Semgrep sink-clear.)
