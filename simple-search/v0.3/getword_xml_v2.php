...
$letter_alts=array();
$letter_alts['f'] = ['p','ph'];
function generate_letter_alts($letter) {
...
function generate_hkalternates($wordin) {
 $ans=[];
 $dbg=true;
 $letters = str_split($wordin);
 dbgprint($dbg,"wordin -> " . join(',',$letters) . '\n');
 $letter_alts = [];
 foreach($letters as $letter) {
  $letter_alts[] = generate_letter_alts($letter);
 }
 $prevwords = [''];
 $n = strlen($wordin);
 for($i=0;$i<$n;$i++) {
