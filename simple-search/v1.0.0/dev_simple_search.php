...
class Simple_Search{
 ...
 public function __construct($keyin0,$dict) {
  ...
  // Linearize searchdict keys into keysin 
  $this->keysin = [];
  foreach($this->searchdict as $k=>$v){
   $this->keysin[] = $k;
  }
  ...
  */
  ...
  $varChars = $this->getChar($word);
  dbgprint($this->dbg,"doVariant: varChars=" . join(',',$varChars) . "\n");
  /* This logic removes 1 (one) character from $word, and recurses.
     Should it remove len($varChar) characters ?
  */
  if (strlen($word) > 1) {
   /// This is confusing. Why do we do this?
   ...
   if ($isExist == true) {
    foreach($cases as $newChar) {
     ...
     if (! $this->ngramValidate($pref1)) {
      dbgprint($this->dbg,"Ngram drop: $pref1\n");
      continue;
     }
 ...
 public function old_getChar($word) {
  /* word is assumed to be an SLP1 spelling.
